...
function full() {
  ...
  NODES.forEach((n, i) => {
    ...
    s += `<rect x="${nodeX}" y="${y}" width="${nodeW}" height="${nodeH}" fill="${INK.washi}" stroke="${ink}" stroke-width="1.5" stroke-linejoin="miter"/>`;
    s += `<text x="${nodeX + 12}" y="${y + 21}" fill="${ink}" font-family="${MONO}" font-size="13" font-weight="600">${n.title}</text>`;
    s += `<text x="${nodeX + 12}" y="${y + 37}" fill="${tierInk70(n.tier)}" font-family="${MONO}" font-size="11">${n.label}</text>`;
  ...
  // decomposition (full only): elbow from node-5 bottom → Think node → agent task
  const n5bx = cx, n5by = yBot(4);
  const thX = 120, thY = 474, thW = 64, thH = 26;
  const thCx = thX + thW / 2;
  s += `<path d="M${n5bx} ${n5by} L${n5bx} ${455} L${thCx} ${455} L${thCx} ${thY - 6}" fill="none" stroke="${INK.sumi}" stroke-width="1.25"/>`;
  ...
  return svg(300, 512, s, "The Kamae chain — five documents, thinking tier into doing tier");
}
...
function header() {
  ...
  return svg(nodeX + nodeW + 8, yBot(4) + 6, s, "The Kamae chain (compact)");
}
...
export function chainSvg(scale = "full") {
