...
export function parseGlossary() {
  ...
  const entries = [];
  // boundary = the next entry HEADER, which is always a bold headword + em dash preceded by a
  // blank line. Requiring the blank line (\n\n) is load-bearing: some definitions carry
  // line-starting **bold** sub-labels that are themselves followed by an em dash (e.g. the
  // "hot / cold" entry's `**hot** —` / `**cold** —` sub-definitions). Those sit on continuation
  // lines with no preceding blank line, so the \n\n guard keeps them inside their parent entry
  // instead of truncating it (which would drop the parent's citation and _Plain:_ cut).
  const re = /(^|\n)\*\*(.+?)\*\*\s+—\s+([\s\S]*?)(?=\n\n\*\*[^*\n]+?\*\*\s+—|\n---|\s*$)/g;
  let m;
  while ((m = re.exec(body))) {
    ...
    entries.push({ term, anchor: slug(term), html: md.renderInline(def), plain, rawDef: def });
  }
  return entries;
}

// ─── term node pages (Basis §13) ──────────────────────────────────────────────
// One node per glossary term, derived from the REAL glossary — no authored content:
//   summary      = the _Plain:_ cut (depth 1)
//   depths 2–3   = the full definition, split into sentence-halves (§13 "split sensibly")
//   related      = bold cross-terms that are themselves glossary anchors (→ node #depth-2)
//   backlinks    = terms whose definition marks this term (derived "referenced by"), never authored
//   requires     = the defining-document tokens from the trailing citation parenthetical
//   cites        = the same defining documents, rendered as the §13 citations line (with §fragments)
//   entry_points = the alias forms that open this term's define-on-touch card (the alias table)
// GOLD appears nowhere (§13). All links are the §5 solid-bero affordance.

// split prose into sentences without breaking on decimals (2.8), section refs (§1.4) or
// lowercase abbreviations (e.g., i.e.): a boundary is a period after a lowercase letter or a
// closing quote/paren/bracket, then whitespace, then an uppercase letter / quote / emphasis.
...
const DEPTH_TAG = { 1: "plain", 2: "definition", 3: "usage" };

export function buildTermNodes(githubBase) {
  const entries = parseGlossary();
  const { aliases } = termIndex();
  const resolve = buildCitationResolver(githubBase);
  const anchorSet = new Set(entries.map((e) => e.anchor));
  const termByAnchor = Object.fromEntries(entries.map((e) => [e.anchor, e.term]));

  // alias forms per anchor → entry_points (the touch-forms that open this term's card)
  const entryPointsByAnchor = {};
  for (const a of aliases) (entryPointsByAnchor[a.key] ??= []).push(a.form);
  // alias regexes per anchor → derived backlinks (which OTHER defs reference this term).
  // Case-INSENSITIVE here: "referenced by" is a conceptual relationship, so a sentence-initial
  // "Forward-only" counts the same as a mid-sentence "forward-only" (the define-on-touch pass
  // stays case-sensitive; this derivation is about concept references, not marked occurrences).
  const aliasesByAnchor = {};
  for (const a of aliases) (aliasesByAnchor[a.key] ??= []).push(new RegExp(a.re.source, "i"));

  const nodes = entries.map((entry) => {
    const { term, anchor, plain } = entry;
    let def = entry.rawDef;

    // peel the trailing citation parenthetical (_(doc §sec; doc; …)_) off the definition
    let citationInner = "";
    const cm = def.match(/\s*_\(([^_]+)\)_\s*$/);
    if (cm) { citationInner = cm[1]; def = def.slice(0, cm.index).trim(); }

    // requires / cites — resolvable defining-document tokens from that parenthetical
    const cites = [];
    for (const seg of citationInner.split(";")) {
      const tokM = seg.match(/[a-z][a-z0-9]*(?:-[a-z0-9]+)+(?:\.md)?/);
      if (!tokM) continue;
      const secM = seg.match(/§\s*(\d+(?:\.\d+)*)/);
      const text = tokM[0].replace(/\.md$/, "");
      const href = resolve(tokM[0], secM ? secM[1] : null);
      if (!href) continue;                       // unresolvable → not a real doc, drop
      if (!cites.some((c) => c.text === text)) cites.push({ text, href, section: secM ? secM[1] : null });
    }

    // related — bold cross-terms that are glossary anchors (not self); land at depth 2
    const related = [];
    const seenRel = new Set();
    for (const bm of def.matchAll(/\*\*([^*]+)\*\*/g)) {
      const key = slug(bm[1]);
      if (!key || key === anchor || seenRel.has(key) || !anchorSet.has(key)) continue;
      seenRel.add(key);
      related.push({ anchor: key, text: termByAnchor[key], href: `/glossary/${key}/#depth-2` });
    }

    // depths — 1 = plain cut; the full definition split into halves for 2 (& 3 when it splits)
    const used = new Set();
    const depths = [{ level: 1, tag: DEPTH_TAG[1], html: md.renderInline(plain) }];
    const sentences = splitSentences(def);
    if (sentences.length <= 1) {
      depths.push({ level: 2, tag: DEPTH_TAG[2], html: markTerms(md.renderInline(def), used) });
    } else {
      const mid = Math.ceil(sentences.length / 2);
      depths.push({ level: 2, tag: DEPTH_TAG[2], html: markTerms(md.renderInline(sentences.slice(0, mid).join(" ")), used) });
      depths.push({ level: 3, tag: DEPTH_TAG[3], html: markTerms(md.renderInline(sentences.slice(mid).join(" ")), used) });
    }

    return {
      anchor, id: anchor, term, type: "definition",
      summary: plain,
      requires: cites.map((c) => c.text),
      requiresLinks: cites,
      cites,
      related,
      entryPoints: entryPointsByAnchor[anchor] || [],
      depths,
      maxDepth: depths[depths.length - 1].level,
      url: `/glossary/${anchor}/`,
      crumb: [{ id: "glossary", href: "/glossary/" }, { id: anchor, current: true }],
    };
  });

  // derived backlinks ("referenced by") — B is referenced by A when A's definition marks B.
  // Derived from the alias index, never authored; forward-only (a page cites, it never claims
  // to be cited). Uses the SAME alias regexes the define-on-touch pass uses.
  for (const node of nodes) {
    const regexes = aliasesByAnchor[node.anchor] || [];
    node.backlinks = entries
      .filter((e) => e.anchor !== node.anchor && regexes.some((re) => { re.lastIndex = 0; return re.test(e.rawDef); }))
      .map((e) => ({ anchor: e.anchor, text: e.term, href: `/glossary/${e.anchor}/#depth-2` }));
  }
...
export function buildTermIndex() {
  ...
  for (const e of entries) {
    ...
    const key = e.anchor;
    // §13: the card's "full entry →" now lands on the term's node page at depth 2.
    plainCuts[key] = { plain: e.plain, fullHref: `/glossary/${key}/#depth-2`, fullLabel: "full entry →" };
...
// resolve a cited document token (± a §section) to an on-site page (with a heading
// fragment when the section maps to a real id) or a GitHub-source fallback (§5). Shared by
// the glossary page's citation linker and the term-node builder so both resolve identically.
export function buildCitationResolver(githubBase) {
